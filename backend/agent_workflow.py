...
def run(s, revision, mode='local', consent=False, call_model=None, persist=lambda r:None):
    ...
    try:
        ...
        if mode=='local':
            ...
            report['explanation']=(
                'The local fixed plan inspected the overnight-hub network, then simulated all three strategies. '
                'Four separate scores expose the trade-offs; only plans passing the hard constraints can be approved. '
                'There is no single weighted winner. No language model ran.'
            )
        else:
